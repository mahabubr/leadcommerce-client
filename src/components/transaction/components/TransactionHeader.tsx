...
const TransactionHeader = ({ data }: Props) => {
    return (
        <Flex align="center" justify='space-between' style={{ marginTop: '10px', gap: '5px' }}>
            ...
            <div>
                <Paragraph style={{ margin: 0 }}><CalendarOutlined />  {moment(data?.createdAt).format('DD.MM.YYYY, HH:mm')}</Paragraph>
                <Paragraph style={{ margin: 0, color: '#adb5bd' }}>Transaction ID: {data?.payment_code}</Paragraph>
                <Paragraph style={{ margin: 0, color: '#adb5bd' }}>Transaction Hash: {data?.order_id}</Paragraph>
